...
function getExchangeRate() {
  return Common.nativeFindOne({ handle: 'actual-group' }, Site).then(function(site) {
...
function getMethodGroupsWithTotals(quotationId, activeStore, options) {
  ...
  return Promise.all(totalsPromises)
    .then(function(totalsPromises) {
      return [totalsPromises, getExchangeRate()];
    })
...
function getPaymentGroupsForEmail(quotation, activeStore) {
  return getMethodGroupsWithTotals(quotation, activeStore).then(function(res) {
    var cash = [
      {
        name: 'Pago único',
...
var paymentGroups = [
  ...
  //       type: '3-msi',
  //       msi: 3,
  //       cardsImages: ['/cards/amexcard.png', '/cards/banamex.png', '/cards/bancomer.png'],
  //       cards: [
  //         'Afirme',
  //         'American Express',
  //         'Banamex',
