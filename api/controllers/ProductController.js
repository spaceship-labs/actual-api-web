...
module.exports = {
  async demandedweb(req, res) {
    const { storeCode } = req.allParams();
    try {
      const itemcodes = await ProductService.getMostDemandedOnWeb(storeCode);
      return res.json(itemcodes);
    } catch (err) {
  ...
  findBySlug: function(req, res) {
    ...
    var query = {
      or: [{ Name: new RegExp(slug, 'i') }, { ItemName: new RegExp(slug, 'i') }]
    };
